...
std::vector<sf::Color> getPixels(const sf::Image& source) {
  auto pixelArray{source.getPixelsPtr()}; // pointer to read-only color array (R,G,B,Alpha)
  auto size = source.getSize();
  auto n    = size.x * size.y; // number of pixels
  std::vector<sf::Color> pixels{};
  for (unsigned int i{0}; i < n * 4; i += 4) {
    pixels.push_back({pixelArray[i], pixelArray[i + 1], pixelArray[i + 2], pixelArray[i + 3]});
  }
  // controlla che il numero dei pixel combacia
  return pixels;
}

sf::Image resize(const sf::Image& source) {
  unsigned int N{64};
  ...
  float yConv{static_cast<float>(sourceY - 1) / static_cast<float>(N - 1)};
  ...
  for (unsigned int x{0}; x < N; ++x) {
    for (unsigned int y{0}; y < N; ++y) {
      ...
      unsigned int y1{static_cast<unsigned int>(yF)};
      ...
      sf::Color Q22{source.getPixel(x2, y2)};
      ...
      float yWeight{yF - static_cast<float>(y1)};

      sf::Color C1{linErp(Q11.r, Q21.r, xWeight), linErp(Q11.g, Q21.g, xWeight), linErp(Q11.b, Q21.b, xWeight),
                   linErp(Q11.a, Q21.a, xWeight)}; // Q11-Q21 linear interp

      sf::Color C2{linErp(Q12.r, Q22.r, xWeight), linErp(Q12.g, Q22.g, xWeight), linErp(Q12.b, Q22.b, xWeight),
                   linErp(Q12.a, Q22.a, xWeight)}; // Q12-Q22 linear interp

      sf::Color C{linErp(C1.r, C2.r, yWeight), linErp(C1.g, C2.g, yWeight), linErp(C1.b, C2.b, yWeight),
...
std::vector<int> binaryConvert(sf::Image source) { // converts resized image to binary
  ...
  for (long unsigned int i{0}; i < pixels.size(); ++i) {
    int colorAvg{static_cast<int>((pixels[i].r + pixels[i].b + pixels[i].g) / 3)};
    if (colorAvg >= 127) {
      binary.push_back(+1);
    } else {
      binary.push_back(-1);
